...
Open the appropriate NDVI script in the Earth Engine Code Editor.
Replace boundaryAsset with the path to the study-region boundary.
Change regionNameField if the region-name attribute is not Field.
Set year to the required year.
Click Run in the Earth Engine Code Editor.
Check the Console for the number of Landsat scenes and the region names.
Open the Tasks tab and click Run for each generated export task.
Repeat these steps for each required year.
...
Open gee_dem_slope.js in the Earth Engine Code Editor.
Replace boundaryAsset with the path to the study-region boundary.
Change regionNameField if the region-name attribute is not Field.
Click Run.
Open the Tasks tab and manually start the elevation and slope export tasks for every study region.
